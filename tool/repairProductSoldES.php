<?php
     ...
     * author: xinde
     * 
     * 修复商品脚本(nohup php tool/repairProductSoldES.php -t "all" -w "userId>12052" -m "50000" > /data/kongsearch_logs/res.log &)
     *****************************************/
    ...
    $cmdopts = getopt('t:u:w:m:p:g:z:h');
    ...
    if($isRepairSpider) {
        $gatherlogpath = '/data/kongsearch_logs/esSpider_productSold_indexupdate.log';
    } else {
        $gatherlogpath = '/data/kongsearch_logs/es_productSold_indexupdate.log';
    }
    $confpath = '/data/project/kongsearch/conf/indexupdate.ini';
    $config = IndexUpdate::getConfig($confpath);
    $indexkey = 'product_sold';
    ...
    $searchIndex = 'item_sold';
    $searchType = 'product';
    $maxLoad    = '45.0';
    
    $redis = '192.168.1.137:6379';
    $repairKey = 'IndexUpdate:repairProductSoldES';
    $expire    = 86400; //一天
    ...
    function repairSingle($userId, $isModify=0, $isStrict=0)
    {
        ...
        $condition['filter']['must'][] = array('field' => 'salestatus', 'value' => 1);
        ...
        if($isStrict == 0 && $searchNum == $dbItemNum) { //在不严格检测下数量相等则认为数据一致
            ...
            echo "Current Table saledItem_{$tableId} Is All Right !!! \n";
            return 0;
        }
...
//        if(false) {
        if($searchHasOnlyNum > 0 && $delTotal <= $maxDeleteNum) { //将只有搜索有的数据删除掉
            $tmp = 0;
...
//                ElasticSearchModel::updateDocument($searchHost, $searchPort, $searchIndex, $searchType, $itemid, array('isdeleted' => 1));
            ...
            echo "The Delete Num Is Gather Then $maxDeleteNum , Exit.\n";
            exit;
        }
