<?php
    ...
    $user = 'repair';
    $password = '';
    $repairKey = 'IndexUpdate:repairProduct';
    $expire    = 86400; //一天
    ...
    function repairSingle($userId, $isModify=0, $isStrict=0)
    {
        ...
        $searchNumSql    = "SELECT count(*) AS num FROM product WHERE userid=$userId AND isdeleted=0 AND certifystatus=1 AND shopstatus=1"; //在搜索中数量
        ...
        if($searchNum < 10000) { //从搜索中取得符合条件的商品ID
            $getSearchIdSql    = "SELECT id FROM product WHERE userid=$userId AND isdeleted=0 AND certifystatus=1 AND shopstatus=1 LIMIT 10000 OPTION max_matches=10000";
            ...
            $startId = 0;
            for($i = 1; $i <= $times; $i++) {
                $getSearchIdSql    = "SELECT id FROM product WHERE userid=$userId AND isdeleted=0 AND certifystatus=1 AND shopstatus=1 AND id>$startId ORDER BY id ASC LIMIT 10000 OPTION max_matches=10000";
                $getSearchIdQuery  = mysql_query($getSearchIdSql, $link1);
                while($getSearchIdResult = mysql_fetch_assoc($getSearchIdQuery)) {
                    $searchIdArr[] = $getSearchIdResult['id'];
                }
