...
if("graphic".equals(viewStyle)){
    tplName = "tpl_book_graphic.jsp" ;
    viewListRows = "50";
}else{
    tplName = "tpl_book.jsp";
    viewListRows = "100";
}
...
// System.out.println("flag:" + flag + "\t"+ act);
...
if("".equals(isNewBook)){ isNewBook = "-1"; }
...
if(!"Mozilla".equalsIgnoreCase(StringUtils.subStr(agent, 7)) 
    && !"Opera".equalsIgnoreCase(StringUtils.subStr(agent, 5))){
    System.out.println(agent+"\t"+pageNo+"\t"+query);
}

String preciseResNum = StringUtils.strVal(request.getParameter("preciseResNum"));
String fuzzyResNum   = StringUtils.strVal(request.getParameter("fuzzyResNum"));

String readCache = StringUtils.strVal(request.getHeader("readCache"));
...
if(null != manager){
    ...
    //普通查询加强版
    //parameters.put("advanced", "extend");
    //查询参数
    ...
    // 其它参数
    //parameters.put("userAgent", agent);
    parameters.put("readCache", readCache);
    // 混合搜索时候的参数
    parameters.put("preciseResNum", preciseResNum);
    parameters.put("fuzzyResNum",   fuzzyResNum);
    ...
    if(null != resultSet){
        ...
        preciseResNum = StringUtils.strVal(resultSet.get("preciseResNum"));
        fuzzyResNum   = StringUtils.strVal(resultSet.get("fuzzyResNum"));
    }else{
    ...
    if("0".equals(result)){
        ...
        keywords = StringUtils.strVal(resultSet.get("keywords"));
        serverStatus = "ok";
...
if("ok".equals(serverStatus)){
    if(("fuzzy".equals(searchMode) || "mix".equals(searchMode)) && StringUtils.intVal(bookTotal) > 0){
        // 精确搜索不到结果时或者混合搜索时候的提示信息
...
if("mix".equals(searchMode) && !queryStr.contains("preciseResNum")){
 queryStr = queryStr +"&preciseResNum="+preciseResNum+"&fuzzyResNum="+fuzzyResNum;
}
String htmlPageNavigation = displayNavigation(navPageTotal, StringUtils.intVal(currentPage), queryStr, "book.jsp");
...
request.setAttribute("onlyStall", onlyStall);
...
request.setAttribute("preciseResNum", preciseResNum);
request.setAttribute("fuzzyResNum", fuzzyResNum);


//System.out.println("book,"+viewStyle+","+sorttype+","+pageNo+","+isNewBook+","+state+","+county);
