...
<%!
    ...
    private void displayHitsTableForDelete(ArrayList itemList, JspWriter out, String docBaseType) throws Exception
    {
        ...
        buffer.append("<td bgcolor=\"#D8D8D8\" width=\"12%\">操作</td>");
        ...
        for(int i = 0, length = itemList.size(); i < length; i++){
            ...
            String catId        = StringUtils.strVal(map.get("catId"));
            String categoryName = getBookCategory(catId);
            ...
            buffer.append(getBizTypeDesc(bizType) +"&nbsp;</td>");
           ...
           // +"【personReject："+personReject+"】" 
            ...
            buffer.append("<td width=\"8%\"><a href=\"javascript:queryCategory('"+catId+"')\">" + categoryName +"</a>&nbsp;</td>");
    ...
    ****************************************************************************/
    ...
    if(null != manager){
        ...
        if("Assist".equalsIgnoreCase(act)){
            ...
            if("ReadUnauditedBook".equalsIgnoreCase(assistTask)){
                //调用远程查询接口
                try{
            ...
            }else if ("BuildFrozenBook".equalsIgnoreCase(assistTask)){
                //调用远程查询接口
                try{
