...
//回调函数类型
String backgroungType = request.getParameter("searchKeywordBackgroungFnType");
//之前选中的分类级别
...
//第三级中所有的选项
String hiddenKeywordAllOptions = request.getParameter("search_group_info_find_all_keyword_none_info_value_dom_hidden");
if(null == hiddenKeywordAllOptions){
	hiddenKeywordAllOptions = "";
} else {
	try {//防乱码
		hiddenKeywordAllOptions = URLEncoder.encode(hiddenKeywordAllOptions, "UTF-8");
	} catch (Exception e) {
		e.printStackTrace();
	}
}
...
<script type="text/javascript">
	...
	$j(function() {//jquery-加载器
		...
		/** 回调函数类型 [group = 二级回调,key = 三级回调]*/
		var backgroungFnType = "<%=backgroungType%>";
		...
		function initializtion(){
			if("<%=showKeyName%>" != ""){
				...
				useContainerControlLoadOnlinePage(showDom,"<%=groupLevel%>","<%=showKeyName%>",1,"<%=groupId%>",
						"<%=hiddenKeyword%>","<%=hiddenKeywordAllOptions%>");
		...
		$j("[name='search_choice_keywords_dom']").live("click", function() {
			...
			backgroungFnType = _this.attr("backgroungFnType");//回调类型[group = 二级回调,key = 三级回调]
		...
		*/
		function useContainerControlLoadOnlinePage(control,nLevel,showDomName,actV,groupInfoID,threeOption,hiddenThreeOption){
			control.load("<%=basePath%>admin/keywords/readKeywords.jsp",{
				...
				searchBackgroungFnType:backgroungFnType,//* 回调函数类型
				...
				hiddenThreeOption:hiddenThreeOption//第三级的选项值的隐藏值
			});
		}
		...
		//分类级别或关键字分类改变事件
		$j("[name='search_group_level_select_dom'],[name='search_group_info_select_dom'],[name='search_key_word_select_dom'],[name='search_group_info_find_all_keyword_none_info']").die();
		$j("[name='search_group_level_select_dom'],[name='search_group_info_select_dom'],[name='search_key_word_select_dom'],[name='search_group_info_find_all_keyword_none_info']").live("change",function(){
...
//									labelDom.hide();//隐藏(无)
									...
									if(childOptions.length > 0){
										//得到第三级的显示值存储控件，并将值存至该控件中
										var childrenDom = $j("[name='search_group_info_find_all_keyword_none_info_value_dom_hidden']");
										childrenDom.val(back);
										labelDom.find("option").remove();//删除子控件选中的值
										labelDom.append("<option value=''>--请选择--</option>");
										$j(childOptions).each(function(ei,ed){
											labelDom.append("<option value='"+ed+"'>"+ed+"</option>");
										});
									}//在执行回调函数之前执行，用户界面刷新后保存当前值
									//开始执行回调函数
									if(backgroungFnType == "group"){//二级回调
			...
			} else {//不需要查询子项数据的控件，即第三级控件，关键词选择框
				if(backgroungFnType == "key"){//三级回调
					if(_this.val() != ""){
						eval(backgroungFnName + "('" + _this.val() + "')");
					}
				}
			}
