...
if("assistant".equals(act)){
    ...
    if("updateKeywords".equals(task)){
    	...
    	try {
            FileWriter writer = new FileWriter(path+"/admin/keywords/version.txt", false);
            writer.write(times);
            writer.close();
        } catch (IOException e) {
            e.printStackTrace();
    	...
    	//写文件
        updater.updateDistrustKeywordsFile(path+"/admin/keywords/distrust_keywords_"+times+".js");
    	//删除多余文件
        File file = new File(path+"/admin/keywords");
        if (file.exists() && file.isDirectory()) {
        	for(File f : file.listFiles()){
        		if (f.isFile() && f.getName().startsWith("distrust_keywords_") && !f.getName().equals("distrust_keywords_"+times+".js")){
        			f.delete();
        		}
        	}
        }
...
<title>设置qqqqqqqqqqqqqqqqqqq</title>
<style>
    body{ 
    margin:0; 
    padding:0; 
