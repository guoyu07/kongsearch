...
if("graphic".equals(viewStyle)){
    tplName = "tpl_book_graphic_old.jsp" ;
    viewListRows = "50";
}else{
    tplName = "tpl_book_old.jsp";
    viewListRows = "100";
}
...
try{
    //取得远程服务器接口实例
    manager = (ServiceInterface) Naming.lookup("rmi://192.168.1.3:9007/BookSearchService");
}catch(Exception ex){
    manager = null;
    ex.printStackTrace();
}
...
if(null != querySuggestList && querySuggestList.size() > 0){
    ...
    for(String word : querySuggestList){
        String link = "<a href=\"book_old.jsp?query="+java.net.URLEncoder.encode(word,"UTF-8")+"\">"+encodeForHTML(word)+"</a>";
        linkList.add(link);
    }
...
String htmlPageNavigation = displayNavigation(navPageTotal, StringUtils.intVal(currentPage), request.getQueryString(), "book_old.jsp");
...
/*if("precise".equals(searchMode) && StringUtils.intVal(bookTotal) < 20){
    queryLessHtml = "<div class=\"queryMore\">"
                  + "提示：如果您想查找更多相关图书，请查看<a href=\"book.jsp?"+request.getQueryString()+"&fuzzy=1\">模糊搜索结果。</a><br />"
