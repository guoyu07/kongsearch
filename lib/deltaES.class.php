...
class DeltaES {
    ...
    public function init($datatype='', $primarydb=NULL) 
    {
        ...
        foreach ($this->config['db'] as $dbname => $dsn) {
            ...
            $waitTimeout = 600;  // MySQL保持空闲连接的最大时间
            if($this->mode == self::MODE_UPDATE) {
                $needInit = 0;
                if($pdb != $dbname) {
        ...
        try {
            ...
            if(isset($this->config['process']) && !empty($this->config['process']))
                $proc = $this->config['process'];
            $this->processor = new Process($dataName, $dataType, $convArgs, self::MODE_UPDATE, $proc);
    ...
    public function insertRecord($record,$id)
    {
        ...
        if($record['isdelete']) {
            $result = ElasticSearchModel::deleteDocument($ip, $port, $indexName, $indexType, $record['itemid'], 1);
//            $this->writeLog('delete document '. $record['itemid']);
        } else {
            unset($record['isdelete']);
    ...
    public function getRecord($fromtable, $pk)
    {
        ...
        foreach($resultset as $row) {
            ...
            if($record === false) {
                $this->errorinfo = "record[{$rid}] process failure:". $dao->getErrorInfo();
                $this->writeLog($this->errorinfo);
                return false;
            } else if(empty ($record)) {
