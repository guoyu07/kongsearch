...
/**
 * 此类同gather.class.php几乎一样，差别在入库后的回写，目的是同索引更新那套机制区分开
 */
...
class Delta {
    ...
    // 返回一个search表数组： array(mod=>db, ...);
    // $k: DB.db1, $v: [n-m] 0 1
    private static function getSearchTables($k, $v) 
    {
        $tables = array();
        $t = explode(' ', $v);
        if ($t === false)
     ...
     *        'searchdb' => array('tablename' => ,'difftable' => ,'shardmod' => , 'tables' => array(mod=>dbname)),
     *        'searchdb_new' => array('tablename' => ,'difftable' => ,'shardmod' => , 'tables' => array(mod=>dbname)),
    ...
    public static function getConfig($inifile)
    {
        ...
        foreach($ini['primary'] as $k => $v) {
            if (strncmp($k, "DB.", 3) == 0) {
                ...
                $env = 'SPHINX_NODE';
                $name = getenv($env);
                if(strpos($datatype, '.') === false && !empty($v)) {
                    $config['primary']['wheres'][$datatype] = $v;
                } else if (strpos($datatype, $name. '.') !== false) {
                    $datatype = substr($datatype, strpos($datatype, '.') + 1);
                    $config['primary']['wheres'][$datatype] = $v;
                }
        ...
        // searchdb
        if(!isset($ini['searchdb']) || empty($ini['searchdb'])) {
            echo "[searchdb] isn't set in configuration.\n";
            return false;
        }

        $config['searchdb']['tables'] = array();
        $config['searchdb']['difftable'] = 0;
        $config['searchdb']['shardmod'] = 1;
        $config['searchdb']['tablename'] = '';
        foreach($ini['searchdb'] as $k => $v) {
            if (strncmp($k, "DB.", 3) == 0) {
                $tables = self::getSearchTables($k, $v);
                if ($tables === false) {
                    echo "$k set error in configuration [searchdb].\n";
                    return false;
                }
                $config['searchdb']['tables'] += $tables;
            } else if(strcasecmp($k, 'difftable') == 0){
                $config['searchdb']['difftable'] = intval($v);
            } else if(strcasecmp($k, 'shardmod') == 0) {
                $config['searchdb']['shardmod'] = intval($v);
            } else if(strcasecmp($k, 'tablename') == 0) {
                $config['searchdb']['tablename'] = $v;
            }
        }

        if(empty($config['searchdb']['tablename'])) {
            echo "search table isn't set.\n";
            return false;
        }
        
        // searchdb_new
        if(!isset($ini['searchdb_new']) || empty($ini['searchdb_new'])) {
            echo "[searchdb_new] isn't set in configuration.\n";
            return false;
        }
        
        $config['searchdb_new']['tables'] = array();
        $config['searchdb_new']['difftable'] = 0;
        $config['searchdb_new']['shardmod'] = 1;
        $config['searchdb_new']['tablename'] = '';
        foreach($ini['searchdb_new'] as $k => $v) {
            if (strncmp($k, "DB.", 3) == 0) {
                $tables = self::getSearchTables($k, $v);
                if ($tables === false) {
                    echo "$k set error in configuration [searchdb_new].\n";
                    return false;
                }
                $config['searchdb_new']['tables'] += $tables;
            } else if(strcasecmp($k, 'difftable') == 0){
                $config['searchdb_new']['difftable'] = intval($v);
            } else if(strcasecmp($k, 'shardmod') == 0) {
                $config['searchdb_new']['shardmod'] = intval($v);
            } else if(strcasecmp($k, 'tablename') == 0) {
                $config['searchdb_new']['tablename'] = $v;
            }
        }

        if(empty($config['searchdb_new']['tablename'])) {
            echo "search_new table isn't set.\n";
            return false;
        }
        ...
        //writeback
        if(isset($ini['writeback']) && !empty($ini['writeback'])) {
            $config['writeback'] = $ini['writeback'];
        }
    ...
    public function init($datatype='', $primarydb=NULL) 
    {
        ...
        foreach ($this->config['db'] as $dbname => $dsn) {
            ...
            if($this->mode == self::MODE_UPDATE) {
                ...
                if($pdb != $dbname) {
                    ...
                    if(!$needInit) {
                        foreach($this->config[$this->searchdb]['tables'] as $dbname2) {
                            if($dbname == $dbname2) {
                                $needInit = 1;
                                break;
                            }
        ...
        $searchtable = $this->config[$searchdb]['tablename'];
        foreach ($this->config[$searchdb]['tables'] as $mod => $dbname) {
            $dao = $this->daos[$dbname];
            if($this->config[$searchdb]['difftable'])
                $table = $searchtable . $mod;
            else 
                $table = $searchtable;
            if($this->mode == self::MODE_UPDATE)
                $stmt = $dao->prepareInsert($table, $cols, 'REPLACE');
            else
                $stmt = $dao->prepareInsert($table, $cols);
            if($stmt === false) {
                $error = $dao->getErrorInfo();
                $this->errorinfo = "prepare [{$table}] insert statement failure: $error";
                $this->writeLog($this->errorinfo);
                return false;
            }
            $this->stmts[$mod] = $stmt;
        ...
        // 处理初始化
        try {
            $dataName = $this->config['data']['name'];
            $dataType = '';
    ...
    {
        $mod = $id % $this->config[$this->searchdb]['shardmod'];       
        $dbname = $this->config[$this->searchdb]['tables'][$mod];
        $dao = $this->daos[$dbname];
        $stmt = $this->stmts[$mod];
        ...
        $r = $dao->executeInsert($stmt,$values);
        if($r === false) {
            $this->errorinfo = $dao->getErrorInfo();
            $this->writeLog($this->errorinfo);
            return false;
        } else if($r === 1062) { // 插入记录的主键重复，记录日志。
            $this->errorinfo = $dao->getErrorInfo();
            $this->writeLog($this->errorinfo);
        }
...
//                    return false;
                    ...
                    if(isset($this->config['writeback']) && $this->config['writeback']['iswriteback'] == 1) {
                        $resultset = $dao->update($table, $this->config['writeback']['updateattr'], $rid, '', $this->config['writeback']['primarykey']);
                        if($resultset === false) {
                            $this->errorinfo = $dao->getErrorInfo();
                            $this->writeLog($this->errorinfo);
                        }
